...
# Size gate: real onefile PyInstaller binary >= 5 MB
$sizeMB = (Get-Item $src).Length / 1MB
if ($sizeMB -lt 5) {
    throw "Backend exe is only $([math]::Round($sizeMB, 1)) MB - PyInstaller produced an empty/broken binary. Common causes: run_server.py missing, spec pathex wrong, SKIP list too aggressive."
}
...
if (Test-Path $envExample) {
    ...
    Set-Content -Path "$ResourceDir\.env.example" -Value "# Configure via Settings page" -Encoding utf8
}
...
$strayExe = "$PSScriptRoot\target\release\ai-producer-hub-backend.exe"
