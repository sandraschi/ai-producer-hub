...
def setup_midi_tools(mcp: FastMCP):
    ...
    async def record_midi_performance(
        device_index: int = 0, duration_seconds: int = 30, output_file: str = ""
    ) -> dict:
        """
        ...
        try:
            ...
            if device_index >= len(ports):
                return {"success": False, "error": f"Invalid device index. Available: {ports}"}
            ...
            while (asyncio.get_event_loop().time() - start_time) < duration_seconds:
                ...
                if msg:
                    ...
                    recorded_events.append(
                        {"message": message, "time": asyncio.get_event_loop().time() - start_time}
                    )
    ...
    async def send_midi_note(
        device_index: int, note: int, velocity: int = 100, duration_ms: int = 500, channel: int = 0
    ) -> dict:
        """
    ...
    @mcp.tool()
    async def midi_to_reaper(midi_file: str, track_name: str = "MIDI Recording") -> dict:
        """
    ...
    async def midi_to_ai_seed(
        midi_file: str, expansion_style: str = "elaborate", target_duration: int = 120
    ) -> dict:
        """
    ...
    @mcp.tool()
    async def play_midi_file(midi_file: str, device_index: int = 0) -> dict:
        """
    ...
    @mcp.tool()
    async def midi_monitor(device_index: int = 0, duration_seconds: int = 10) -> dict:
        """
